...
class LinksScraper():
    ...
    def gather_links(self, depth):
        """ 
        Gather all the links we want to scrape data from to train our bot 
        
        Arguments:
            depth: depth of links scraping (1 equals scraping links from the sublinks only)

        """

        # Get the institutions links since they don't have a main page
        institutions_links = self.get_institutions_links()

        # Open links file to scrape
        with open(self.links_file.value, 'r') as file:
            links = [line.strip() for line in file]

        links = set(links)
        links_to_scrape = set(links.union(institutions_links))
            ...
            for link in current_links:
                ...
                if link not in total_links:
                    ...
                    try:
                        ...
                        new_links = self.gather_sublinks(link, soup)
        ...
        # Write to file to avoid scraping every time
        with open(Path.ScrapedLinks.value, 'wb') as file:
            pickle.dump(total_links, file)

        return total_links

    def get_institutions_links(self):
        ...
        # Search for the Institution in the menu bar
    ...
    def gather_sublinks(self, main_link, soup):
